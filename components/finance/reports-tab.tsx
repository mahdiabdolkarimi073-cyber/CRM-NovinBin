...
interface ReportsTabProps {
  ...
  treasurySummary: {
    totalCash: number; totalBank: number; totalTreasury: number;
    pendingChequesReceived: number; pendingChequesPaid: number;
  } | null;
    ...
    <div className="space-y-4">
      ...
      {reportType === 'summary' && summary && (
        <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
          ...
          <Card><CardContent className="p-4 flex items-center justify-between">
            <div><div className="text-xs text-slate-400">حقوق صاحبان سهام</div><div className="text-xl font-bold text-violet-600">{formatToman(summary.totalEquity)} ت</div></div>
          ...
          <Card><CardContent className="p-4 flex items-center justify-between">
            <div><div className="text-xs text-slate-400">سود خالص</div><div className={`text-xl font-bold ${summary.netProfit >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>{formatToman(summary.netProfit)} ت</div></div>
            ...
            <CardContent className="p-0">
              <table className="w-full text-sm">
                <tbody className="divide-y divide-slate-100">
                  ...
                  <tr className="bg-slate-50 font-bold"><td className="p-3">مجموع</td><td className="p-3 text-left text-red-700">{formatToman((summary?.totalLiabilities || 0) + (summary?.totalEquity || 0))}</td></tr>
                </tbody>
              </table>
            </CardContent>
          </Card>
          ...
          <CardContent className="p-0">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                ...
                <tr className="bg-slate-50 font-bold"><td className="p-3" colSpan={2}>سود خالص</td><td className={`p-3 text-left ${(summary?.netProfit || 0) >= 0 ? 'text-emerald-700' : 'text-red-700'}`}>{formatToman(summary?.netProfit || 0)}</td></tr>
              </tbody>
            </table>
          </CardContent>
        </Card>
        ...
        <Card>
          <CardContent className="p-0">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-slate-100">
                {trialBalance.map(({ account, debit, credit }) => (
                  <tr key={account.id} className="hover:bg-slate-50">
                    <td className="p-3 font-mono text-slate-500" dir="ltr">{account.code}</td>
                    <td className="p-3">{account.name}</td>
                    <td className="p-3 text-left font-medium text-sky-600">{debit > 0 ? formatToman(debit) : '—'}</td>
                    <td className="p-3 text-left font-medium text-red-600">{credit > 0 ? formatToman(credit) : '—'}</td>
                  </tr>
                ))}
      ...
      {reportType === 'treasury' && treasurySummary && (
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <Card><CardContent className="p-4 flex items-center justify-between">
            <div><div className="text-xs text-slate-400">مجموع نقدی</div><div className="text-xl font-bold text-emerald-600">{formatToman(treasurySummary.totalCash)} ت</div></div>
          ...
          <Card><CardContent className="p-4 flex items-center justify-between">
            <div><div className="text-xs text-slate-400">مجموع بانکی</div><div className="text-xl font-bold text-sky-600">{formatToman(treasurySummary.totalBank)} ت</div></div>
          ...
          <Card><CardContent className="p-4 flex items-center justify-between">
            <div><div className="text-xs text-slate-400">چک‌های دریافتی در جریان</div><div className="text-xl font-bold text-emerald-600">{formatToman(treasurySummary.pendingChequesReceived)} ت</div></div>
          ...
          <Card><CardContent className="p-4 flex items-center justify-between">
            <div><div className="text-xs text-slate-400">چک‌های پرداختی در جریان</div><div className="text-xl font-bold text-red-600">{formatToman(treasurySummary.pendingChequesPaid)} ت</div></div>
            <div className="w-10 h-10 rounded-xl bg-red-50 text-red-600 flex items-center justify-center"><BookOpen className="w-5 h-5" /></div>
          </CardContent></Card>
        </div>
      )}
    </div>
