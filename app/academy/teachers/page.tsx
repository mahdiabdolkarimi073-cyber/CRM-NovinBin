...
type Teacher = {
  ...
  active: boolean; createdAt: string; specialtyCount: number; scheduleCount: number;
  activeCourses: number; totalSettled: number; totalPending: number; documentCount: number; evaluationCount: number;
};
...
export default function TeachersPage() {
  ...
  const fetchTeachers = useCallback((q: string) => {
    fetch(`/api/academy/teachers?q=${encodeURIComponent(q)}`, { headers: { 'Cache-Control': 'no-store' } })
      .then(async (res) => { if (res.ok) { const data = await res.json(); setTeachers(data.teachers || []); } else if (res.status === 403) router.replace('/academy/login'); })
      .finally(() => setLoading(false));
  }, [router]);

  useEffect(() => { fetchTeachers(''); }, [fetchTeachers]);
  useEffect(() => { const t = setTimeout(() => fetchTeachers(search), 300); return () => clearTimeout(t); }, [search, fetchTeachers]);
  ...
  async function save() {
    ...
    try {
      ...
      if (editTeacher) { payload.id = editTeacher.id; if (!payload.password) delete payload.password; }
      const res = await fetch('/api/academy/teachers', { method: editTeacher ? 'PUT' : 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(payload) });
      const data = await res.json();
      if (!res.ok) { setError(data.error || 'خطا در ذخیره'); return; }
      setShowModal(false); fetchTeachers(search);
    } finally { setSaving(false); }
  }

  async function toggleActive(t: Teacher) {
    await fetch('/api/academy/teachers', { method: 'PUT', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ id: t.id, active: !t.active }) });
      ...
      <div className="academy-admin-main">
        ...
        <div className="academy-admin-scroll">
          <div className="academy-admin-page-hero">
            <div>
              <h2>مدیریت مدرس‌ها</h2>
              <p>ایجاد، ویرایش و مدیریت مدرس‌های آموزشگاه</p>
            </div>
            ...
            <button type="button" className="academy-admin-btn-primary" onClick={openCreate}><Plus /> مدرس جدید</button>
          </div>

          {loading ? (
            <div className="academy-admin-loading-inline"><Loader2 className="animate-spin" /></div>
          ) : teachers.length === 0 ? (
            <div className="academy-admin-list-empty"><Users /><p>مدرسی یافت نشد.</p></div>
          ) : (
            <div className="academy-admin-table-wrap">
              <table className="academy-admin-table">
                <thead>
                  <tr>
                    <th>نام و نام خانوادگی</th><th>نام کاربری</th><th>تلفن</th>
                    <th>تخصص‌ها</th><th>کلاس‌های فعال</th><th>تسویه شده</th><th>در انتظار</th>
                    <th>مدارک</th><th>وضعیت</th><th>عملیات</th>
                  </tr>
                </thead>
                <tbody>
                  {teachers.map((t) => (
                    <tr key={t.id}>
                      ...
                      <td>{t.username}</td>
                      <td>{t.phone || '—'}</td>
                      <td>{t.specialtyCount.toLocaleString('fa-IR')}</td>
                      <td>{t.activeCourses.toLocaleString('fa-IR')}</td>
                      <td>{formatMoney(t.totalSettled)}</td>
                      <td>{formatMoney(t.totalPending)}</td>
                      <td>{t.documentCount.toLocaleString('fa-IR')}</td>
                      <td><span className={`academy-admin-badge ${t.active ? 'badge-success' : 'badge-error'}`}>{t.active ? 'فعال' : 'غیرفعال'}</span></td>
                      <td>
                        <div className="academy-admin-row-actions">
                          <button type="button" title="مشاهده پرونده" onClick={() => router.push(`/academy/teachers/${t.id}`)}><Eye /></button>
                          <button type="button" title="ویرایش" onClick={() => openEdit(t)}><Edit2 /></button>
                          <button type="button" title={t.active ? 'غیرفعال کردن' : 'فعال کردن'} onClick={() => toggleActive(t)}>
                            {t.active ? <UserX /> : <UserCheck />}
                          </button>
                        </div>
                      </td>
                    </tr>
        ...
        <div className="academy-admin-modal-overlay" onClick={() => setShowModal(false)}>
          <div className="academy-admin-modal" onClick={(e) => e.stopPropagation()}>
            <div className="academy-admin-modal-header">
              <h3>{editTeacher ? 'ویرایش مدرس' : 'ایجاد مدرس جدید'}</h3>
            ...
            <div className="academy-admin-modal-footer">
              ...
              <button type="button" className="academy-admin-btn-primary" onClick={save} disabled={saving}>
                {saving ? <Loader2 className="animate-spin" /> : null} {editTeacher ? 'ذخیره تغییرات' : 'ایجاد مدرس'}
              </button>
            </div>
          </div>
        </div>
