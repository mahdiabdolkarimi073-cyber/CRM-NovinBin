    ...
    <section className="academy-auth-panel">
      <div className="academy-login-card">
        ...
        <form onSubmit={handleSubmit} className="academy-form">
          ...
          <div className="academy-input-wrap"><LockKeyhole /><input id="academy-password" type={showPassword ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} placeholder="رمز عبور" autoComplete="current-password" required /><button type="button" onClick={() => setShowPassword((value) => !value)} aria-label="نمایش رمز عبور">{showPassword ? <EyeOff /> : <Eye />}</button></div>
