...
import {
  ...
  CheckCircle, Clock, Eye, Ban, FileText, Building2,
} from 'lucide-react';
import { formatJalali, formatToman, relativeTime } from '@/lib/format';
import { fullName } from '@/lib/constants';
import { toast } from 'sonner';
import type {
  SalesReturnInvoice, SalesReturnInvoiceItem, SalesReturnInvoiceHistory,
  Customer, Warehouse, Profile,
} from '@/lib/types';

const SRI_STATUS: Record<string, string> = {
  draft: 'پیش‌نویس',
  confirmed: 'تأیید شده',
  voided: 'باطل شده',
};

const SRI_STATUS_COLOR: Record<string, string> = {
  draft: '#94a3b8',
  confirmed: '#10b981',
  voided: '#ef4444',
};
...
export default function SalesReturnInvoicesPage() {
  const { profile } = useAuth();
  const [records, setRecords] = useState<SalesReturnInvoice[]>([]);
  ...
  const [detail, setDetail] = useState<SalesReturnInvoice | null>(null);
  const [detailItems, setDetailItems] = useState<SalesReturnInvoiceItem[]>([]);
  const [detailHistory, setDetailHistory] = useState<SalesReturnInvoiceHistory[]>([]);
  const [historyDialog, setHistoryDialog] = useState<SalesReturnInvoice | null>(null);
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [sriData, custData, whData, staffData] = await Promise.all([
        fetchData<SalesReturnInvoice>('sales_return_invoices', {
      ...
      toast.error('بارگذاری فاکتورهای برگشت ناموفق: ' + error.message);
    }
    setLoading(false);
  }, []);
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((r) => {
      const matches = !q || r.number?.toLocaleLowerCase().includes(q) || r.customerName?.toLocaleLowerCase().includes(q);
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('حذف این فاکتور برگشت؟')) return;
    try {
      await deleteData('sales_return_invoices', { id });
      toast.success('فاکتور برگشت حذف شد');
      setDetail(null);
      loadData();
    } catch (error: any) {
  ...
  const handleStatusChange = async (id: string, newStatus: string, action: string) => {
    if (!profile) return;
    try {
      await updateData('sales_return_invoices', { id }, {
        status: newStatus,
  ...
  const handleVoid = async () => {
    ...
    try {
      await updateData('sales_return_invoices', { id: voidDialog.id }, {
        status: 'voided',
        voidedBy: profile.id,
  ...
  const loadDetail = (r: SalesReturnInvoice) => {
    setDetail(r);
    setDetailItems(r.items || []);
    setDetailHistory(r.history || []);
  };
    ...
    <div className="w-full" dir="rtl">
      <header className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-[28px] font-bold text-[#101828]">فاکتور برگشت از فروش</h1>
          </div>
          <div className="mt-2 text-xs font-medium text-[#667085]">داشبورد <span className="mx-1.5 text-[#CBD5E1]">←</span> فروش <span className="mx-1.5 text-[#CBD5E1]">←</span> فاکتور برگشت از فروش</div>
        </div>
        <Link href="/dashboard/sales-return-invoices/new">
          <Button className="h-[42px] rounded-[10px] bg-[#3155E7] px-[18px] text-sm font-semibold text-white shadow-sm hover:bg-[#2445C7]">
            <Plus className="h-4 w-4" /> ثبت فاکتور برگشت
          </Button>
        </Link>
      </header>
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="h-[42px] rounded-[10px] border border-[#DCE3EE] bg-white px-3 text-sm text-[#344054]">
          ...
          {Object.entries(SRI_STATUS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
        </select>
      </div>
      ...
      {loading ? (
        ...
        <Card><EmptyState icon={<RotateCcw className="h-8 w-8" />} title="فاکتوری یافت نشد" description="برای شروع، اولین فاکتور برگشت از فروش را ثبت کنید" action={<Link href="/dashboard/sales-return-invoices/new"><Button><Plus className="h-4 w-4" /> افزودن فاکتور</Button></Link>} /></Card>
      ) : (
        <Card><CardContent className="p-0">
          <div className="divide-y divide-[#F1F5F9]">
            {pageItems.map((r) => {
              const stColor = SRI_STATUS_COLOR[r.status] || '#64748b';
              return (
                <div key={r.id} className="flex cursor-pointer items-center gap-3 p-4 transition-colors hover:bg-[#F8FAFD]" onClick={() => loadDetail(r)}>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline" className="shrink-0 text-[10px]" style={{ color: stColor, borderColor: `${stColor}35`, backgroundColor: `${stColor}10` }}>{SRI_STATUS[r.status]}</Badge>
                    </div>
      ...
      <Dialog open={!!detail} onOpenChange={(o) => !o && setDetail(null)}>
        <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
          {detail && (() => {
            const stColor = SRI_STATUS_COLOR[detail.status] || '#64748b';
            return (
              <>
                <DialogHeader>
                  <div className="flex items-start justify-between gap-3">
                    <DialogTitle className="text-lg">فاکتور برگشت {detail.number}</DialogTitle>
                ...
                <div className="space-y-4">
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge style={{ backgroundColor: `${stColor}20`, color: stColor }}>{SRI_STATUS[detail.status]}</Badge>
                  </div>
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                    ...
                    <div className="rounded-[10px] bg-[#F1F5F9] p-3"><div className="text-xs text-[#667085]">مشتری</div><div className="mt-1 text-sm font-bold text-[#344054]">{detail.customerName || customerName(detail.customerId)}</div></div>
                    ...
                    <div className="rounded-[10px] bg-[#F1F5F9] p-3"><div className="text-xs text-[#667085]">ایجادکننده</div><div className="mt-1 text-sm font-bold text-[#344054]">{staffNameById(detail.createdBy)}</div></div>
                  ...
                  {detail.returnReason && <div className="rounded-lg border border-amber-200 bg-amber-50 p-3"><div className="text-xs font-semibold text-amber-700">دلیل برگشت:</div><p className="mt-1 whitespace-pre-wrap text-sm text-amber-600">{detail.returnReason}</p></div>}
                  ...
                  <div className="flex flex-wrap gap-2 border-t border-slate-200 pt-4">
                    ...
                    {isSuperAdmin && detail.status !== 'voided' && (
      ...
      <Dialog open={!!voidDialog} onOpenChange={(o) => !o && setVoidDialog(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>باطل کردن فاکتور برگشت</DialogTitle></DialogHeader>
      ...
      <Dialog open={!!historyDialog} onOpenChange={(o) => !o && setHistoryDialog(null)}>
        <DialogContent className="max-h-[80vh] max-w-lg overflow-y-auto">
          <DialogHeader><DialogTitle>تاریخچه فاکتور برگشت</DialogTitle></DialogHeader>
                ...
                <div key={h.id} className="flex items-start gap-3 rounded-lg border border-slate-100 bg-slate-50 p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 text-xs text-slate-500">
                      توسط {staffNameById(h.actionBy)}
                      {h.fromStatus && h.toStatus && <span> • {SRI_STATUS[h.fromStatus] || h.fromStatus} ← {SRI_STATUS[h.toStatus] || h.toStatus}</span>}
                    </div>
                  </div>
                </div>
              ))}
