...
import {
  ...
  CheckCircle, XCircle, Clock, Eye, Ban, FileText, Building2, Send,
  AlertCircle,
} from 'lucide-react';
...
import type {
  ServicePurchaseInvoice, ServicePurchaseInvoiceItem,
  ServicePurchaseInvoiceHistory, Profile,
} from '@/lib/types';

const SPI_STATUS: Record<string, string> = {
  draft: 'پیش‌نویس',
  service_confirmed: 'تأیید خدمت',
...
export default function ServicePurchaseInvoicesPage() {
  const { profile } = useAuth();
  const [records, setRecords] = useState<ServicePurchaseInvoice[]>([]);
  const [staff, setStaff] = useState<Profile[]>([]);
  const [search, setSearch] = useState('');
  const [filterStatus, setFilterStatus] = useState('all');
  const [page, setPage] = useState(1);
  const [detail, setDetail] = useState<ServicePurchaseInvoice | null>(null);
  const [detailItems, setDetailItems] = useState<ServicePurchaseInvoiceItem[]>([]);
  const [detailHistory, setDetailHistory] = useState<ServicePurchaseInvoiceHistory[]>([]);
  const [historyDialog, setHistoryDialog] = useState<ServicePurchaseInvoice | null>(null);
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [spiData, staffData] = await Promise.all([
        fetchData<ServicePurchaseInvoice>('service_purchase_invoices', {
  ...
  const filtered = useMemo(() => {
    ...
    return records.filter((r) => {
      const matches = !q ||
        r.internalNumber?.toLocaleLowerCase().includes(q) ||
        r.supplierInvoiceNo?.toLocaleLowerCase().includes(q) ||
        r.supplierName?.toLocaleLowerCase().includes(q);
  ...
  const stats = useMemo(() => ({
    ...
    pending: records.filter((r) => r.status === 'draft' || r.status === 'service_confirmed').length,
    approved: records.filter((r) => r.status === 'finance_approved' || r.status === 'posted').length,
    totalAmount: records.filter((r) => r.status !== 'voided').reduce((sum, r) => sum + Number(r.finalAmount || 0), 0),
  }), [records]);

  const handleDelete = async (id: string) => {
    if (!confirm('حذف این فاکتور؟')) return;
    try {
      await deleteData('service_purchase_invoices', { id });
  ...
  const handleStatusChange = async (id: string, newStatus: string, action: string, extra: any = {}) => {
    if (!profile) return;
    try {
      const updates: any = { status: newStatus, updatedAt: new Date().toISOString() };
      if (newStatus === 'service_confirmed') {
        updates.serviceConfirmed = true;
        updates.serviceConfirmedBy = profile.id;
        updates.serviceConfirmedAt = new Date().toISOString();
      }
      if (newStatus === 'finance_approved') {
        updates.financeApproved = true;
        updates.financeApprovedBy = profile.id;
        updates.financeApprovedAt = new Date().toISOString();
      }
      Object.assign(updates, extra);
      await updateData('service_purchase_invoices', { id }, updates);
  ...
  const handleVoid = async () => {
    ...
    try {
      await updateData('service_purchase_invoices', { id: voidDialog.id }, {
        status: 'voided',
        voidedBy: profile.id,
  ...
  const loadDetail = (spi: ServicePurchaseInvoice) => {
    setDetail(spi);
    setDetailItems(spi.items || []);
    setDetailHistory(spi.history || []);
  };
    ...
    <div className="w-full" dir="rtl">
      <header className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-[28px] font-bold text-[#101828]">فاکتور خرید خدمات</h1>
          </div>
          <div className="mt-2 text-xs font-medium text-[#667085]">داشبورد <span className="mx-1.5 text-[#CBD5E1]">←</span> خدمات <span className="mx-1.5 text-[#CBD5E1]">←</span> فاکتور خرید</div>
        </div>
        <Link href="/dashboard/service-purchase-invoices/new">
      ...
      <div className="mb-5 grid grid-cols-2 gap-4 xl:grid-cols-4">
        ...
        <div className="flex min-h-[120px] flex-col justify-between rounded-[14px] border border-[#E7ECF3] bg-white p-5 shadow-[0_3px_14px_rgba(20,40,80,.05)]">
          ...
          <div><div className="text-[20px] font-bold leading-none text-[#101828]">{formatToman(stats.totalAmount)}</div><div className="mt-1.5 text-[13px] font-bold text-[#344054]">مبلغ کل (تومان)</div></div>
        </div>
      </div>
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div className="relative">
          ...
          <Input placeholder="جستجو بر اساس شماره یا تأمین‌کننده..." value={search} onChange={(e) => setSearch(e.target.value)} className="h-[42px] w-full rounded-[10px] border-[#DCE3EE] bg-white pr-9 text-sm sm:w-[320px]" />
        </div>
        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="h-[42px] rounded-[10px] border border-[#DCE3EE] bg-white px-3 text-sm text-[#344054]">
          ...
          {Object.entries(SPI_STATUS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
        </select>
      </div>
      ...
      {loading ? (
        ...
        <Card><EmptyState icon={<FileSearch className="h-8 w-8" />} title="فاکتوری یافت نشد" description="برای شروع، اولین فاکتور خرید خدمات را ثبت کنید" action={<Link href="/dashboard/service-purchase-invoices/new"><Button><Plus className="h-4 w-4" /> افزودن فاکتور</Button></Link>} /></Card>
      ) : (
        <Card><CardContent className="p-0">
          <div className="divide-y divide-[#F1F5F9]">
            {pageItems.map((r) => {
              const stColor = SPI_STATUS_COLOR[r.status] || '#64748b';
              return (
                <div key={r.id} className="flex cursor-pointer items-center gap-3 p-4 transition-colors hover:bg-[#F8FAFD]" onClick={() => loadDetail(r)}>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline" className="shrink-0 text-[10px]" style={{ color: stColor, borderColor: `${stColor}35`, backgroundColor: `${stColor}10` }}>{SPI_STATUS[r.status]}</Badge>
                      <Badge variant="outline" className="shrink-0 text-[10px] text-[#667085]">{PURCHASE_TYPE[r.purchaseType] || r.purchaseType}</Badge>
                    </div>
                    <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-[#98A2B3]">
                      <span className="flex items-center gap-1"><Building2 className="h-3 w-3" />{r.supplierName || '—'}</span>
                      <span className="flex items-center gap-1"><Calendar className="h-3 w-3" />{formatJalali(r.invoiceDate)}</span>
      ...
      <Dialog open={!!detail} onOpenChange={(o) => !o && setDetail(null)}>
        <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
          {detail && (() => {
            const stColor = SPI_STATUS_COLOR[detail.status] || '#64748b';
            return (
              <>
                <DialogHeader>
                ...
                <div className="space-y-4">
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge style={{ backgroundColor: `${stColor}20`, color: stColor }}>{SPI_STATUS[detail.status]}</Badge>
                    <Badge variant="outline" className="text-[#667085]">{PURCHASE_TYPE[detail.purchaseType] || detail.purchaseType}</Badge>
                  </div>
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                    ...
                    <div className="rounded-[10px] bg-[#F1F5F9] p-3"><div className="text-xs text-[#667085]">شماره تأمین‌کننده</div><div className="mt-1 text-sm font-bold text-[#344054]">{detail.supplierInvoiceNo || '—'}</div></div>
                    <div className="rounded-[10px] bg-[#F1F5F9] p-3"><div className="text-xs text-[#667085]">تأمین‌کننده</div><div className="mt-1 text-sm font-bold text-[#344054]">{detail.supplierName || '—'}</div></div>
                    <div className="rounded-[10px] bg-[#DCFCE7] p-3"><div className="text-xs text-[#667085]">تاریخ فاکتور</div><div className="mt-1 text-sm font-bold text-[#16A34A]">{formatJalali(detail.invoiceDate)}</div></div>
                    ...
                    <div className="rounded-[10px] bg-[#F1F5F9] p-3"><div className="text-xs text-[#667085]">مانده</div><div className="mt-1 text-sm font-bold text-[#344054]">{formatToman(Number(detail.balanceDue))}</div></div>
                  </div>

                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-4">
                    <div className="rounded-lg bg-slate-50 p-3 text-center"><div className="text-xs text-slate-500">جمع کل</div><div className="mt-1 text-sm font-bold text-slate-700">{formatToman(Number(detail.subtotal))}</div></div>
                    <div className="rounded-lg bg-slate-50 p-3 text-center"><div className="text-xs text-slate-500">تخفیف</div><div className="mt-1 text-sm font-bold text-rose-500">{formatToman(Number(detail.totalDiscount))}</div></div>
                    <div className="rounded-lg bg-slate-50 p-3 text-center"><div className="text-xs text-slate-500">مالیات</div><div className="mt-1 text-sm font-bold text-amber-600">{formatToman(Number(detail.totalTax))}</div></div>
                    <div className="rounded-lg bg-blue-50 p-3 text-center"><div className="text-xs text-blue-500">مبلغ نهایی</div><div className="mt-1 text-sm font-bold text-blue-700">{formatToman(Number(detail.finalAmount))}</div></div>
                  </div>
                      ...
                      <div className="space-y-1.5">
                        {detailItems.map((item) => (
                          <div key={item.id} className="flex items-center justify-between rounded-lg bg-slate-50 p-3 text-xs">
                            <div className="min-w-0 flex-1">
                              <div className="font-semibold text-slate-700">{item.serviceName || item.description || '—'}</div>
                              <div className="mt-0.5 flex gap-3 text-slate-400">
                                ...
                                <span>مبلغ: {formatToman(Number(item.finalPrice))}</span>
                              </div>
                            </div>
                          </div>
                        ))}
                  ...
                  <div className="flex flex-wrap gap-2 border-t border-slate-200 pt-4">
                    {detail.status === 'draft' && isSuperAdmin && (
                      <Button variant="outline" onClick={() => handleStatusChange(detail.id, 'service_confirmed', 'service_confirmed')}><CheckCircle className="h-4 w-4" /> تأیید خدمت</Button>
                    )}
                    {detail.status === 'service_confirmed' && isSuperAdmin && (
                      <Button variant="outline" className="border-green-200 text-green-600 hover:bg-green-50" onClick={() => handleStatusChange(detail.id, 'finance_approved', 'finance_approved')}><CheckCircle className="h-4 w-4" /> تأیید مالی</Button>
                    )}
                    {detail.status === 'finance_approved' && isSuperAdmin && (
                      <Button variant="outline" className="border-blue-200 text-blue-600 hover:bg-blue-50" onClick={() => handleStatusChange(detail.id, 'posted', 'posted')}><Send className="h-4 w-4" /> ثبت سند</Button>
                    )}
                    {isSuperAdmin && detail.status !== 'voided' && detail.status !== 'posted' && (
                ...
                <div key={h.id} className="flex items-start gap-3 rounded-lg border border-slate-100 bg-slate-50 p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 text-xs text-slate-500">
                      ...
                      {h.fromStatus && h.toStatus && <span> • {SPI_STATUS[h.fromStatus] || h.fromStatus} ← {SPI_STATUS[h.toStatus] || h.toStatus}</span>}
                    </div>
                  </div>
                </div>
              ))}
