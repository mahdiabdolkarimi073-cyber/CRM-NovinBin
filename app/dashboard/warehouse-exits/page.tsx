...
import type {
  WarehouseExit, WarehouseExitItem, WarehouseExitHistory,
  Warehouse, Profile,
} from '@/lib/types';

const WE_STATUS: Record<string, string> = {
  draft: 'پیش‌نویس',
  completed: 'تکمیل شده',
...
const WE_STATUS_COLOR: Record<string, string> = {
  draft: '#94a3b8',
  completed: '#06b6d4',
...
const EXIT_TYPE: Record<string, string> = {
  sale: 'فروش',
...
export default function WarehouseExitsPage() {
  const { profile } = useAuth();
  const [records, setRecords] = useState<WarehouseExit[]>([]);
  const [warehouses, setWarehouses] = useState<Warehouse[]>([]);
  const [staff, setStaff] = useState<Profile[]>([]);
  const [search, setSearch] = useState('');
  const [filterStatus, setFilterStatus] = useState('all');
  const [page, setPage] = useState(1);
  const [detail, setDetail] = useState<WarehouseExit | null>(null);
  const [detailItems, setDetailItems] = useState<WarehouseExitItem[]>([]);
  const [detailHistory, setDetailHistory] = useState<WarehouseExitHistory[]>([]);
  const [historyDialog, setHistoryDialog] = useState<WarehouseExit | null>(null);
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [weData, whData, staffData] = await Promise.all([
      ...
      toast.error('بارگذاری خروج‌ها ناموفق: ' + error.message);
    }
    setLoading(false);
  }, []);
  ...
  const stats = useMemo(() => ({
    ...
    pending: records.filter((r) => r.status === 'draft' || r.status === 'pending_approval').length,
  ...
  const handleDelete = async (id: string) => {
    if (!confirm('حذف این خروج؟')) return;
    try {
      await deleteData('warehouse_exits', { id });
      toast.success('خروج حذف شد');
      setDetail(null);
      loadData();
    } catch (error: any) {
  ...
  const handleStatusChange = async (id: string, newStatus: string, action: string) => {
    if (!profile) return;
    try {
      await updateData('warehouse_exits', { id }, {
        status: newStatus,
  ...
  const handleVoid = async () => {
    ...
    try {
      await updateData('warehouse_exits', { id: voidDialog.id }, {
        status: 'voided',
        voidedBy: profile.id,
  ...
  const loadDetail = (we: WarehouseExit) => {
    setDetail(we);
    setDetailItems(we.items || []);
    ...
    <div className="w-full" dir="rtl">
      ...
      <Dialog open={!!detail} onOpenChange={(o) => !o && setDetail(null)}>
        <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
          {detail && (() => {
            const stColor = WE_STATUS_COLOR[detail.status] || '#64748b';
            return (
              <>
                <DialogHeader>
