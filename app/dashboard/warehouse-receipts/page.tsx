...
import {
  ArrowDownToLine, Plus, Search, Trash2, Calendar, ChevronLeft, ChevronRight,
...
import type {
  WarehouseReceipt, WarehouseReceiptItem, WarehouseReceiptHistory,
  Warehouse, Profile,
} from '@/lib/types';

const WR_STATUS: Record<string, string> = {
  draft: 'پیش‌نویس',
  confirmed: 'تأیید شده',
  posted: 'ثبت سند',
  voided: 'باطل شده',
...
const WR_STATUS_COLOR: Record<string, string> = {
  draft: '#94a3b8',
  confirmed: '#10b981',
  posted: '#6366f1',
  voided: '#ef4444',
...
export default function WarehouseReceiptsPage() {
  ...
  const [records, setRecords] = useState<WarehouseReceipt[]>([]);
  ...
  const [detailItems, setDetailItems] = useState<WarehouseReceiptItem[]>([]);
  const [detailHistory, setDetailHistory] = useState<WarehouseReceiptHistory[]>([]);
  const [historyDialog, setHistoryDialog] = useState<WarehouseReceipt | null>(null);
  ...
  const loadData = useCallback(async () => {
    setLoading(true);
    try {
      const [wrData, whData, staffData] = await Promise.all([
      ...
      toast.error('بارگذاری رسیدها ناموفق: ' + error.message);
    }
    setLoading(false);
  }, []);
  ...
  const stats = useMemo(() => ({
    ...
    confirmed: records.filter((r) => r.status === 'confirmed' || r.status === 'posted').length,
    totalValue: records.filter((r) => r.status !== 'voided').reduce((sum, r) => sum + Number(r.totalValue || 0), 0),
  }), [records]);

  const handleDelete = async (id: string) => {
    if (!confirm('حذف این رسید؟')) return;
    try {
      await deleteData('warehouse_receipts', { id });
  ...
  const handleStatusChange = async (id: string, newStatus: string, action: string) => {
    if (!profile) return;
    try {
      await updateData('warehouse_receipts', { id }, {
        status: newStatus,
  ...
  const handleVoid = async () => {
    ...
    try {
      await updateData('warehouse_receipts', { id: voidDialog.id }, {
        status: 'voided',
        voidedBy: profile.id,
  ...
  const loadDetail = (wr: WarehouseReceipt) => {
    setDetail(wr);
    ...
    <div className="w-full" dir="rtl">
      <header className="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <div className="flex items-center gap-3">
            ...
            <h1 className="text-[28px] font-bold text-[#101828]">رسید انبار</h1>
          </div>
          <div className="mt-2 text-xs font-medium text-[#667085]">داشبورد <span className="mx-1.5 text-[#CBD5E1]">←</span> انبارداری <span className="mx-1.5 text-[#CBD5E1]">←</span> رسید انبار</div>
        </div>
        <Link href="/dashboard/warehouse-receipts/new">
          <Button className="h-[42px] rounded-[10px] bg-[#3155E7] px-[18px] text-sm font-semibold text-white shadow-sm hover:bg-[#2445C7]">
            <Plus className="h-4 w-4" /> ثبت رسید
          </Button>
        </Link>
      </header>
      ...
      <div className="mb-5 grid grid-cols-2 gap-4 xl:grid-cols-4">
        <div className="flex min-h-[120px] flex-col justify-between rounded-[14px] border border-[#E7ECF3] bg-white p-5 shadow-[0_3px_14px_rgba(20,40,80,.05)]">
          <span className="flex h-[42px] w-[42px] items-center justify-center rounded-full bg-[#3155E7]/10 text-[#3155E7]"><ArrowDownToLine className="h-5 w-5" strokeWidth={2.5} /></span>
          <div><div className="text-[26px] font-bold leading-none text-[#101828]">{stats.total.toLocaleString('fa-IR')}</div><div className="mt-1.5 text-[13px] font-bold text-[#344054]">کل رسیدها</div></div>
        </div>
        <div className="flex min-h-[120px] flex-col justify-between rounded-[14px] border border-[#E7ECF3] bg-white p-5 shadow-[0_3px_14px_rgba(20,40,80,.05)]">
      ...
      <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        ...
        <select value={filterStatus} onChange={(e) => setFilterStatus(e.target.value)} className="h-[42px] rounded-[10px] border border-[#DCE3EE] bg-white px-3 text-sm text-[#344054]">
          ...
          {Object.entries(WR_STATUS).map(([k, v]) => <option key={k} value={k}>{v}</option>)}
        </select>
      </div>
      ...
      {loading ? (
        ...
        <Card><EmptyState icon={<ArrowDownToLine className="h-8 w-8" />} title="رسیدی یافت نشد" description="برای شروع، اولین رسید انبار را ثبت کنید" action={<Link href="/dashboard/warehouse-receipts/new"><Button><Plus className="h-4 w-4" /> افزودن رسید</Button></Link>} /></Card>
      ) : (
        <Card><CardContent className="p-0">
          <div className="divide-y divide-[#F1F5F9]">
            {pageItems.map((r) => {
              const stColor = WR_STATUS_COLOR[r.status] || '#64748b';
              return (
                <div key={r.id} className="flex cursor-pointer items-center gap-3 p-4 transition-colors hover:bg-[#F8FAFD]" onClick={() => loadDetail(r)}>
                  ...
                  <div className="min-w-0 flex-1">
                    <div className="flex items-center gap-2">
                      ...
                      <Badge variant="outline" className="shrink-0 text-[10px]" style={{ color: stColor, borderColor: `${stColor}35`, backgroundColor: `${stColor}10` }}>{WR_STATUS[r.status]}</Badge>
                      <Badge variant="outline" className="shrink-0 text-[10px] text-[#667085]">{RECEIPT_TYPE[r.receiptType] || r.receiptType}</Badge>
                    </div>
                    <div className="mt-1 flex flex-wrap items-center gap-3 text-xs text-[#98A2B3]">
                      ...
                      <span className="flex items-center gap-1"><Calendar className="h-3 w-3" />{formatJalali(r.receiptDate)}</span>
      ...
      <Dialog open={!!detail} onOpenChange={(o) => !o && setDetail(null)}>
        <DialogContent className="max-h-[85vh] max-w-2xl overflow-y-auto">
          {detail && (() => {
            const stColor = WR_STATUS_COLOR[detail.status] || '#64748b';
            return (
              <>
                <DialogHeader>
                  <div className="flex items-start justify-between gap-3">
                    <DialogTitle className="text-lg">رسید {detail.number}</DialogTitle>
                ...
                <div className="space-y-4">
                  <div className="flex flex-wrap items-center gap-2">
                    <Badge style={{ backgroundColor: `${stColor}20`, color: stColor }}>{WR_STATUS[detail.status]}</Badge>
                    <Badge variant="outline" className="text-[#667085]">{RECEIPT_TYPE[detail.receiptType] || detail.receiptType}</Badge>
                  </div>
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                    ...
                    <div className="rounded-[10px] bg-[#DCFCE7] p-3"><div className="text-xs text-[#667085]">تاریخ رسید</div><div className="mt-1 text-sm font-bold text-[#16A34A]">{formatJalali(detail.receiptDate)}</div></div>
                  ...
                  <div className="flex flex-wrap gap-2 border-t border-slate-200 pt-4">
                    {detail.status === 'draft' && isSuperAdmin && (
                      <Button variant="outline" className="border-green-200 text-green-600 hover:bg-green-50" onClick={() => handleStatusChange(detail.id, 'confirmed', 'confirmed')}><CheckCircle className="h-4 w-4" /> تأیید</Button>
                    )}
                    {detail.status === 'confirmed' && isSuperAdmin && (
      ...
      <Dialog open={!!voidDialog} onOpenChange={(o) => !o && setVoidDialog(null)}>
        <DialogContent className="max-w-md">
          <DialogHeader><DialogTitle>باطل کردن رسید</DialogTitle></DialogHeader>
      ...
      <Dialog open={!!historyDialog} onOpenChange={(o) => !o && setHistoryDialog(null)}>
        <DialogContent className="max-h-[80vh] max-w-lg overflow-y-auto">
          <DialogHeader><DialogTitle>تاریخچه رسید</DialogTitle></DialogHeader>
                ...
                <div key={h.id} className="flex items-start gap-3 rounded-lg border border-slate-100 bg-slate-50 p-3">
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <div className="mt-1 text-xs text-slate-500">
                      ...
                      {h.fromStatus && h.toStatus && <span> • {WR_STATUS[h.fromStatus] || h.fromStatus} ← {WR_STATUS[h.toStatus] || h.toStatus}</span>}
                    </div>
                  </div>
                </div>
              ))}
