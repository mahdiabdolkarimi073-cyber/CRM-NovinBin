...
import { ArrowRight, Building2, Info, Lightbulb, Loader2, Hash, Type as TypeIcon, Layers } from 'lucide-react';
import { toast } from 'sonner';
import type { CostCenter } from '@/lib/types';

const guideItems = [
  { icon: Hash, title: 'کد یکتا', desc: 'کد مرکز هزینه باید منحصربه‌فرد باشد.' },
  { icon: TypeIcon, title: 'نام واضح', desc: 'نام مرکز هزینه را به‌صورت گویا وارد کنید.' },
  { icon: Layers, title: 'سلسله‌مراتب', desc: 'با انتخاب مرکز والد، ساختار درختی بسازید.' },
];
...
export default function NewCostCenterPage() {
  ...
  const loadData = useCallback(async () => {
    try {
      const ccs = await fetchData<CostCenter>('cost_centers', { where: {}, orderBy: { code: 'asc' } });
      setCostCenters(ccs || []);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!form.code.trim() || !form.name.trim()) { toast.error('کد و نام مرکز هزینه الزامی است'); return; }
    setSubmitting(true);
    try {
      await createData('cost_centers', {
        code: form.code.trim(),
        name: form.name.trim(),
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <h1 className="text-[24px] font-bold leading-tight text-[#0F172A] sm:text-[32px]">ایجاد مرکز هزینه</h1>
          </div>
          <p className="mt-2 text-[14px] text-[#64748B]">مرکز هزینه جدید برای ردیابی هزینه‌ها تعریف کنید</p>
        </div>
      ...
      <div className="mt-8 grid grid-cols-1 gap-7 lg:grid-cols-[2fr_0.9fr]">
        <div className="rounded-[14px] border border-[#DCE4EF] bg-white p-5 shadow-[0_4px_18px_rgba(15,23,42,0.05)] sm:p-7">
          <div className="mb-6">
            <div className="flex items-center gap-2">
              <Building2 className="h-[21px] w-[21px] text-[#2563EB]" />
              <h2 className="text-[20px] font-bold text-[#0F172A]">اطلاعات مرکز هزینه</h2>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div className="grid grid-cols-1 gap-5 sm:grid-cols-2">
              <div>
                <Label className="mb-2 block text-[14px] font-semibold text-[#172033]">کد <span className="text-[#DC2626]">*</span></Label>
                <Input dir="ltr" value={form.code} onChange={(e) => setForm({ ...form, code: e.target.value })} placeholder="مثال: 100" className="h-[50px] rounded-[10px] border-[#D4DEEA] text-[14px] focus:border-[#2563EB] focus:shadow-[0_0_0_3px_rgba(37,99,235,0.10)]" required />
              </div>
              <div>
                <Label className="mb-2 block text-[14px] font-semibold text-[#172033]">نام <span className="text-[#DC2626]">*</span></Label>
                <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="مثال: واحد فروش" className="h-[50px] rounded-[10px] border-[#D4DEEA] text-[14px] focus:border-[#2563EB] focus:shadow-[0_0_0_3px_rgba(37,99,235,0.10)]" required />
              </div>
            </div>
            <div>
              <Label className="mb-2 block text-[14px] font-semibold text-[#172033]">مرکز هزینه والد</Label>
            ...
            <div className="flex flex-row gap-3.5 pt-2">
              <Button type="submit" disabled={submitting} className="h-[52px] w-[60%] rounded-[10px] bg-[#102A68] text-[14px] font-bold text-white transition-all hover:bg-[#1a3a7a] hover:shadow-md sm:w-[175px]">
                {submitting ? (<><Loader2 className="h-4 w-4 animate-spin" /> در حال ایجاد...</>) : 'ایجاد مرکز هزینه'}
              </Button>
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-[12px] border border-[#BFDBFE] bg-[#EFF6FF] p-5 sm:p-6">
            ...
            <p className="text-[13px] leading-[2] text-[#64748B]">مراکز هزینه به شما کمک می‌کنند هزینه‌ها را به‌تفکیک واحد سازمانی ردیابی و گزارش‌گیری کنید.</p>
          </div>
        </div>
      </div>
    </div>
