...
import { ArrowRight, Banknote, Info, Lightbulb, Loader2, MapPin } from 'lucide-react';
import { toast } from 'sonner';

const guideItems = [
  { icon: Banknote, title: 'نام صندوق', desc: 'نام صندوق را به‌صورت گویا وارد کنید.' },
  { icon: MapPin, title: 'محل صندوق', desc: 'محل فیزیکی صندوق را مشخص کنید.' },
];

export default function NewCashFundPage() {
  const { profile } = useAuth();
  const router = useRouter();
  const [submitting, setSubmitting] = useState(false);
  const [form, setForm] = useState({ name: '', type: 'cash', location: '', balance: '' });

  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!profile) { toast.error('اطلاعات کاربر بارگذاری نشده'); return; }
    if (!form.name.trim()) { toast.error('نام صندوق الزامی است'); return; }
    setSubmitting(true);
    try {
      await createData('cash_funds', {
        name: form.name.trim(),
        type: form.type,
      ...
      <div className="flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
        <div>
          <div className="flex items-center gap-2.5">
            ...
            <h1 className="text-[24px] font-bold leading-tight text-[#0F172A] sm:text-[32px]">ایجاد صندوق نقدی</h1>
          </div>
          <p className="mt-2 text-[14px] text-[#64748B]">صندوق نقدی یا فروشگاهی جدید ثبت کنید</p>
        </div>
      ...
      <div className="mt-8 grid grid-cols-1 gap-7 lg:grid-cols-[2fr_0.9fr]">
        <div className="rounded-[14px] border border-[#DCE4EF] bg-white p-5 shadow-[0_4px_18px_rgba(15,23,42,0.05)] sm:p-7">
          <div className="mb-6">
            <div className="flex items-center gap-2">
              <Banknote className="h-[21px] w-[21px] text-[#2563EB]" />
              <h2 className="text-[20px] font-bold text-[#0F172A]">اطلاعات صندوق</h2>
            </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-5">
            <div>
              <Label className="mb-2 block text-[14px] font-semibold text-[#172033]">نام صندوق <span className="text-[#DC2626]">*</span></Label>
              <Input value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} placeholder="مثال: صندوق فروشگاه" className="h-[50px] rounded-[10px] border-[#D4DEEA] text-[14px] focus:border-[#2563EB] focus:shadow-[0_0_0_3px_rgba(37,99,235,0.10)]" required />
            </div>
            <div>
              <Label className="mb-2 block text-[14px] font-semibold text-[#172033]">نوع صندوق</Label>
            ...
            <div>
              <Label className="mb-2 block text-[14px] font-semibold text-[#172033]">محل</Label>
              <Input value={form.location} onChange={(e) => setForm({ ...form, location: e.target.value })} placeholder="مثال: دفتر مرکزی" className="h-[50px] rounded-[10px] border-[#D4DEEA] text-[14px] focus:border-[#2563EB] focus:shadow-[0_0_0_3px_rgba(37,99,235,0.10)]" />
            </div>
            <div>
            ...
            <div className="flex flex-row gap-3.5 pt-2">
              <Button type="submit" disabled={submitting} className="h-[52px] w-[60%] rounded-[10px] bg-[#102A68] text-[14px] font-bold text-white transition-all hover:bg-[#1a3a7a] hover:shadow-md sm:w-[175px]">
                {submitting ? (<><Loader2 className="h-4 w-4 animate-spin" /> در حال ایجاد...</>) : 'ایجاد صندوق'}
              </Button>
        ...
        <div className="space-y-5">
          ...
          <div className="rounded-[12px] border border-[#BFDBFE] bg-[#EFF6FF] p-5 sm:p-6">
            ...
            <p className="text-[13px] leading-[2] text-[#64748B]">صندوق‌های نقدی برای مدیریت وجه نقد سازمان استفاده می‌شوند و می‌توانید بین آن‌ها و حساب‌های بانکی انتقال وجه انجام دهید.</p>
          </div>
        </div>
      </div>
    </div>
