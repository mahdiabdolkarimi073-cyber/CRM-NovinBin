...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    if (file.type && !ALLOWED_TYPES.includes(file.type) && !file.type.startsWith('image/')) {
      return NextResponse.json({ error: 'تنها فایل تصویری مجاز است' }, { status: 400 });
    }
    ...
    const url = `/uploads/meetings/${safeName}`;
    ...
    return NextResponse.json({ error: 'آپلود تصویر ناموفق بود' }, { status: 500 });
  }
}
