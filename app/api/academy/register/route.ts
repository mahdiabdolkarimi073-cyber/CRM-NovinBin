...
export async function POST(req: NextRequest) {
  try {
    ...
    const existing = await (prisma as any).academyUser.findFirst({ where: { username } });
    if (existing) return NextResponse.json({ error: 'این نام کاربری قبلاً ثبت شده است' }, { status: 409 });

    const account = await (prisma as any).academyUser.create({ data: { firstName, lastName, username, phone: body.phone ? String(body.phone).trim() : null, passwordHash: bcrypt.hashSync(password, 10), role: 'student' } });
